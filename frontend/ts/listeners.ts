...
export function attachHomePageListeners() {
	...
	if (match_btn)
		match_btn.addEventListener("click", (event) => { navigate('/pong/select/type'); matchTypeChoice('match', 'pong') });

	const tournament_btn = document.getElementById("tournament-button");
	if (tournament_btn)
		tournament_btn.addEventListener("click", (event) => { navigate(''); showAliasInputs(4, 'tournoi', 'normal', 'pong')});

	const pfc_button = document.getElementById("pfc-button");
	if (pfc_button)
		pfc_button.addEventListener("click", (event) => { navigate('/chifoumi/select/type'); matchTypeChoice('match', 'pfc')});

	const pong_hist_btn = document.getElementById("pong-hist-btn");
	if (pong_hist_btn)
		pong_hist_btn.addEventListener("click", (event) => { navigate('/pong/history/two'); showHistory('pong')});

	const fourpong_hist_btn = document.getElementById("fourpong-hist-btn");
	if (fourpong_hist_btn)
		fourpong_hist_btn.addEventListener("click", (event) => { navigate('/pong/history/four'); showHistory('fourpong')});

	const pfc_hist_btn = document.getElementById("pfc-hist-btn");
	if (pfc_hist_btn)
		pfc_hist_btn.addEventListener("click", (event) => { navigate('/chifoumi/history'); showHistory('pfc')});
